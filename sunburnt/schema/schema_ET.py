...
class SolrSchema(object):
	...
	def schema_parse(self, f):
		try:
			schemadoc = ElementTree.parse(f)
		except ElementTree.XMLSyntaxError, e:
			raise SolrError("Invalid XML in schema:\n%s" % e.args[0])

		field_type_classes = {}
		for field_type_node in schemadoc.findall("/types/fieldType") + schemadoc.findall("/types/fieldtype"):
...
class SolrResponse(object):
	def __init__(self, schema, xmlmsg):
		self.schema = schema
		doc = ElementTree.fromstring(xmlmsg)
		details = dict(value_from_node(n) for n in
					   doc.findall("lst") if n.attrib['name'] != 'moreLikeThis')
		...
		more_like_these_nodes = \
			[n for n in doc.findall("lst/result") if n.attrib['name'] == 'moreLikeThis']
